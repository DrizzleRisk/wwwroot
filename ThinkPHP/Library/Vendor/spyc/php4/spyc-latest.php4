 ...
 */
  ...
  /**
     ...
     */
  function YAMLDump($array, $indent = false, $wordwrap = false, $no_opening_dashes = false) {
    $spyc = new Spyc;
  ...
  /**
     ...
     * @param int $wordwrap Pass in 0 for no wordwrap, false for default (40)
     */
  function dump($array,$indent = false,$wordwrap = false, $no_opening_dashes = false) {
  ...
  /**
     ...
     */
  function _yamlize($key,$value,$indent, $previous_key = -1, $first_key = 0, $source_array = null) {
    if (is_array($value)) {
      if (empty ($value))
  ...
  /**
     ...
     * @param $indent The indent of the current node
     */
  function _dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0, $source_array = null) {
    ...
    if ($value === "'" . $this->REMPTY . "'") $value = null;
    ...
    if (is_array ($source_array) && array_keys($source_array) === range(0, count($source_array) - 1)) {
      ...
      // if ($first_key===0)  trigger_error('Keys are all screwy.  The first one was zero, now it\'s "'. $key .'"',E_USER_ERROR);
      // It's mapped
  ...
  function isTrueWord($value) {
    $words = $this->getTranslations(array('true', 'on', 'yes', 'y'));
    return in_array($value, $words, true);
  }

  function isFalseWord($value) {
    $words = $this->getTranslations(array('false', 'off', 'no', 'n'));
    return in_array($value, $words, true);
  }

  function isNullWord($value) {
    $words = $this->getTranslations(array('null', '~'));
    return in_array($value, $words, true);
  }

  function isTranslationWord($value) {
    return (
  ...
  /**
     ...
     * @access private
     */
  function getTranslations(array $words) {
    $result = array();
    foreach ($words as $i) {
...
// LOADING FUNCTIONS
  ...
  function addLiteralLine ($literalBlock, $line, $literalBlockStyle, $indent = -1) {
  ...
  function checkKeysInValue($value) {
    if (strchr('[{"\'', $value[0]) === false) {
      if (strchr($value, ': ') !== false) {
          trigger_error('Too many keys: '.$value,E_USER_ERROR);
      }
    }
  }
